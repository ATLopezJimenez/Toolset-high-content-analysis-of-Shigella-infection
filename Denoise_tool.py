...
#------------------------------MERGE DENOISED IMAGE WITH OTHER CHANNELS AND SAVE------------------------------#
        ...
        normalised_mask = mask/255 # merge with mask channel
        new_red = bact * normalised_mask

        im_blue = np.zeros ((128,128)) # merge with bacteria channel
        image_merge = np.dstack((new_red, img_f_mask, im_blue))
